...
def covBalATTBootstrapEval(w_c, X, T, metric, prop=0.5, smps=20):
...
def covBalATEBootstrapEval(w_c, X, T, metric, prop=0.5, smps=20):
    '''evaluate covariate balance with bootstrap samples t
    to choose tuning parameters'''
    ...
    if np.all(w_c > -1):
        ...
        cov_difs = [(w_c[subsample].dot(X[T==0][subsample]) / np.sum(w_c[subsample]) - \
            np.mean(X, axis = 0))/sd for subsample in subsamples]
    ...
    for prop in [0.1]:
        for cov_qual in ["good", "bad"]:
            ...
            for objective in ["l1", "l2", "entropy"]:
                ...
                for i in range(N):
                    print(i)                   
                    filename = datdir + str(i+1)+'kangschafer_approxbal_'+cov_qual+'.csv'
                    data = pd.read_csv(filename)

                    cov, T, Y = data.iloc[:,:4].as_matrix().copy(), \
                        data["t_ind"].copy(), data["y"].copy()

                    # only balance the mean of the covariates
                    X = cov

                    true_att = np.mean(Y[T==1])
                    true_ate = np.mean(Y)

                    # wrong_ate = np.mean(Y[T==1]) - np.mean(Y[T==0])
                    ...
                    att_sqerr = [(w_c.dot(Y[T==0]) - true_att)**2 \
                        for w_c in att_w_c_vals]
                    att_sqerrs.append(att_sqerr)

                    ate_w_c_vals = approxBalATE(X, T, Y, tol_vals, objective)

                    ate_covbal_l1 = [covBalATEBootstrapEval(w_c, X, T, "l1", prop=prop) \
                        for w_c in ate_w_c_vals]
                    ate_covbal_l2 = [covBalATEBootstrapEval(w_c, X, T, "l2", prop=prop) \
                        for w_c in ate_w_c_vals]
                    ate_covbal_linf = [covBalATEBootstrapEval(w_c, X, T, "linf", prop=prop) \
                        for w_c in ate_w_c_vals]
                    ...
                    ate_sqerr = [(w_c.dot(Y[T==0]) - true_ate)**2 \
                        for w_c in ate_w_c_vals]
                ...
                att_sqerrs[att_sqerrs > 1e3] = np.nan
                att_covbal_l1s[att_covbal_l1s > 1e3] = np.nan
                att_covbal_l2s[att_covbal_l2s > 1e3] = np.nan
                att_covbal_linfs[att_covbal_linfs > 1e3] = np.nan
                ...
                ate_sqerrs[ate_sqerrs > 1e3] = np.nan
                ate_covbal_l1s[ate_covbal_l1s > 1e3] = np.nan
                ate_covbal_l2s[ate_covbal_l2s > 1e3] = np.nan
                ate_covbal_linfs[ate_covbal_linfs > 1e3] = np.nan

                np.savetxt("KangSchafer_att_sqerr_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", att_sqerrs, delimiter=",")
                np.savetxt("KangSchafer_att_covbal_l1_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", att_covbal_l1s, delimiter=",")
                np.savetxt("KangSchafer_att_covbal_l2_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", att_covbal_l2s, delimiter=",")
                np.savetxt("KangSchafer_att_covbal_linf_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", att_covbal_linfs, delimiter=",")


                np.savetxt("KangSchafer_ate_sqerr_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", ate_sqerrs, delimiter=",")
                np.savetxt("KangSchafer_ate_covbal_l1_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", ate_covbal_l1s, delimiter=",")
                np.savetxt("KangSchafer_ate_covbal_l2_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", ate_covbal_l2s, delimiter=",")
                np.savetxt("KangSchafer_ate_covbal_linf_"+objective+"_"+cov_qual+"_"+str(int(prop*100))+".csv", ate_covbal_linfs, delimiter=",")
            ...
            att_err_vs_covbal.to_csv("KangSchafer_att_err_vs_covbal_"+cov_qual+".csv")
            ate_err_vs_covbal.to_csv("KangSchafer_ate_err_vs_covbal_"+cov_qual+".csv")

            ate_err_vs_covbal[ate_err_vs_covbal>1e3]=np.inf
            att_err_vs_covbal[att_err_vs_covbal>1e3]=np.inf
            ...
            plt.tight_layout()
            fig1.savefig("KangSchafer_ate_err_vs_covbal_"+cov_qual+".pdf")
            ...
            plt.tight_layout()
            fig2.savefig("KangSchafer_att_err_vs_covbal_"+cov_qual+".pdf")
